...
namespace AdventOfCode
{
    ...
    abstract internal class Folder : File
    {
    ...
    internal class ParentFolder : Folder
    {
        public List<Folder> Folders { get; set; }
        public ParentFolder(string name) : base(name)
        {
            Name = name;
            Size = 0;
        ...
        static void Main(string[] args)
        {
            ...
            foreach (var item in data)
            {
                if (item.Contains("$ cd"))
                {
                    if (item.Contains(".."))
                    {
                        structure.RemoveAt(structure.Count - 1);
                    }
                    else
                    {
                        structure.Add(item.Substring(5));
                    }                                       
                }
                ...
                else
                {
                    ...
                    {                        
                        map.Add(new File(item.Substring(item.IndexOf(" ") + 1), 0));
                    }
                    else
                    {
                        foreach (var folder in structure)
                        {
                            foreach (var ma in map)
                            {
                                if (folder == ma.Name)
                                {
                                    ma.Size += long.Parse(item.Substring(0, item.IndexOf(" ")));                                    
                                }
                            }
                        }
                    }
